    ...
    if request.method == 'POST':
        ...
        if mix_layer == 'off':
            ...
            if oral_section!=None:
                ...
                #save the recording files from the front-end
                for i in range(len(voice_text_list)):
                    ...
                    print('save record'+str(i+1)+'_robot.wav')
                    engine.runAndWait()
            ...
            ##write down the configuration into db
        sql = "insert into task (code,task_name,introduction,time_limit,vote_system,mode,award_star,age_group,select_all" \
              ",task_status,character_task_demand, scenario_task_demand, layer1_demand, layer2_demand, mix_layer," \
              "create_time,process_count,net_error,nlp_error,speech_recognition) \
         values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',now(),0,0,0,0)" % (
        ...
        if select_group=='on':
            ...
            cursor.execute(sql)
            sql="update task set group_no='%s' where code='%s'" %(group_no,code)
            cursor.execute(sql)
        ...
        if age_group != '':
            ...
            if amount[0]['count(*)']>0:
                sql_change_code2 = "update child_user set current_task_code='%s' where age>=%s and age<=%s and finish_" \
                                   "task<>'N'" % (code, age_min, age_max)
        ...
        if mix_layer == 'off':
            if mode=='pure_visual':
                ...
                sql_change_user_mode_linear = "update child_user set current_job='all', finish_task='N' where current_" \
                                              "task_code='%s'" % (code)
            ...
            if length%2==0:
                ...
                for name1 in name_odd:
                    print(name1 + "set as layer1")
                    sql_change_job_layer1 = "update child_user set current_job='layer_1', finish_task='N' where " \
                                            "username='%s'" % (
                        name1)
                    cursor.execute(sql_change_job_layer1)
                for name2 in name_even:
                    print(name2 + "set as layer2")
                    sql_change_job_layer2 = "update child_user set current_job='layer_2' , finish_task='N' " \
                                            "where username='%s'" % (
                        name2)
                    cursor.execute(sql_change_job_layer2)

                sql="update child_user set current_job='layer1_2',finish_task='N' where username='%s'" \
                    %(userlist[-1]['username'])
                cursor.execute(sql)
        # initialize finish_task

        # record user info in local file
        sql_get_code_users = "select username,current_job,current_task_code,layer_partner from child_user " \
                             "where current_task_code='%s'" % (
            code)
        ...
        for i in range(len(name_list_1)):
            sql_update_records = "insert into records (username, code,job_type,layer_partner,finish_task,vote_star) " \
                                 "values ('%s','%s','%s','%s','N',0) " % (
...
def character_design():
    ...
    if request.method == 'POST':
        ...
        session['start_time']=""
        session['end_time']=="" ####the time counting is relevant to the frontend function
        print("time use is"+ str(time_use))
        character_name=request.form.get('character_name')
        note_step1=request.form.get('note_step1')
        img_base64 = str(request.form.get('image'))
        img_base64 = img_base64.replace('data:image/png;base64,', '') #base64 format to save the frontend picture
        ...
        cursor.execute(sql)
        sql="update records set character_name='%s',time_use_character='%s',note_step1='%s' where code='%s' and" \
            " username='%s'" %(character_name,time_use,note_step1,session['code'],session['user'])
        cursor.execute(sql)
        db.commit()
        db.close()
...
def scenario_design():
    ...
    if request.method == 'POST':
        ...
        time_use=int(float(session['end_time']-session['start_time']))
        img_base64 = str(request.form.get('image'))
        note_step2=request.form.get('note_step2')
        ...
        cursor.execute(sql)
        sql="update records set time_use_scenario='%s', note_step2='%s' where username='%s' and code" \
            "='%s'" %(time_use,note_step2,session['user'],session['code'])
        cursor.execute(sql)
...
def mix_layer1():
    ...
    if request.method == 'POST':
        ...
        file.write(img_data)
        file.close()
        layer1_description=request.form.get("layer1_description")
        f=open("static/user/"+session['user']+"/"+session['code']+"/layer1_description.txt",'w')
        f.write(layer1_description)
        f.close()
...
def mix_layer2():
    if request.method == 'GET':
        ...
        if ft[0]['finish_task']=='N' and session['current_job']=='layer_2':
            ...
            db.commit()
            db.close()
            f = open("static/user/" + partner_layer1 + '/' + session['code'] + '/layer1_description.txt', 'r')
            layer1_description = f.read()
            f.close()
            return render_template('mix_layer2.html', code=session['code'], minute=info[0]['time_limit'],
                                   ...
                                   layer1_description=layer1_description)
        else:
            f = open("static/user/" + partner_layer1 + '/' + session['code'] + '/layer1_description.txt', 'r')
            layer1_description = f.read()
            f.close()
    ...
    if request.method == 'POST':
        ...
        file.write(img_data)
        file.close()
        layer2_description = request.form.get("layer2_description")
        f = open("static/user/" + session['user'] + "/" + session['code'] + "/layer2_description.txt", 'w')
        f.write(layer2_description)
        f.close()
        db = pymysql.connect(host='gz-cynosdbmysql-grp-df9ep7mf.sql.tencentcdb.com',
                             port=25081,
                             user='root',
        ...
        sql_change_finish_task = "update child_user set finish_task='Y' where username='%s'" % (session['user'])
        cursor.execute(sql_change_finish_task)
        sql = "update records set finish_task='Y',time_use_layer2='%s' where username='%s' and code=" \
              "'%s'" % (time_use,session['user'],session['code'])
        cursor.execute(sql)
        db.commit()
        db.close()
        return redirect(url_for('visual_star'))
...
def collect_record_model(number):
    if request.method=='GET':
        ...
        if (number-1)<index_length:
            return render_template('talk_model.html', voice_text=question_list[number-1], code=session['code'],
                                   index=number, index_next=number+1,index_length=index_length)
        else:
            db = pymysql.connect(host='gz-cynosdbmysql-grp-df9ep7mf.sql.tencentcdb.com',
                                 port=25081,
                                 user='root',
            ...
            cursor.execute(sql)
            sql_change_finsih_task = "update child_user set finish_task='Y' where username='%s' " \
                                     "and current_task_code='%s'" % (
...
def vote():
    ...
    if request.method == 'POST':
        ...
        for name in name_list:
            ...
            sql="update records set vote_star=vote_star+%s+%s where code='%s' and username='%s' " \
                %(character_score,scenario_score,session['code'],name['username'])
            cursor.execute(sql)
            sql="update child_user set score=score+%s+%s where username='%s'" %(character_score,scenario_score,
                                                                                name['username'])
            cursor.execute(sql)
...
##frontend has the function to count down in 5 seconds
...
def child_user_account():
    if request.method == 'GET':
        ...
        return render_template('child_interface.html', username=session['user'], gender=tmp[0]['gender'],
                               age=tmp[0]['age'], current_task_code=current_task_code, task_status=task_status,
                               country=tmp[0]['country'],score=tmp[0]['score'],group=group)
...
@app.route('/task_detail/<code>', methods=['GET'])
        ...
        if result[0]['count(*)']>0: #如果有还没完成任务的人，则给出一个目前的进度
            if mix_layer_switch=='off':
                if mode=='linear':
                    ...
                    db.commit()
                    db.close()
                    return render_template("Participant_Not_finish_mix_off.html", visual_not_finish_amount=result[0]['count(*)'],
                                           oral_not_finish_amount=result[0]['count(*)'],user_info=user_info)
                    ...
                    db.commit()
                    db.close()
                    return render_template("Participant_Not_finish_mix_off.html",visual_not_finish_amount=visual_us_nfinish[0]['count(*)'],
                                           oral_not_finish_amount=oral_us_nfinish[0]['count(*)'],user_info=user_info)
                    ...
                    db.commit()
                    db.close()
                    return render_template("Participant_Not_finish_mix_off.html",visual_not_finish_amount=pure_visual_us_nfinish[0]['count(*)'],
                                           user_info=user_info,oral_not_finish_amount=0)
            else: #mix layer open
                ...
                db.commit()
                db.close()
                return render_template("Participant_Not_finish_mix_on.html",layer1_not_finish_amount=layer1_us_nfinish[0]['count(*)'],
                                       layer2_not_finish_amount=layer2_us_nfinish[0]['count(*)'],mix_us_list=mix_us_list,code=code)
            ...
            if mix_layer_switch=='off':
                ...
                if mode=='linear':
                    ...
                    ##########
                    if s_r[0]['speech_recognition'] ==0:

                        os.system("nohup python3 listen_speech_recognition.py "+code +" &")
                        sql="update task set speech_recognition=1 where code='%s'" %(code)
                        cursor.execute(sql)
                        db.commit()
                        db.close()
                        return redirect(url_for("s_r_progress",code=code))

                    if s_r[0]['speech_recognition'] ==1:
                        return redirect(url_for("s_r_progress",code=code))
                    else: ##if nlp process is over:
                    ############
                        sql="select username,vote_star,character_name,time_use_character,time_use_scenario,note_step1," \
                            "note_step2 from records where code='%s' and job_type='all'" %(code)
                        cursor.execute(sql)
                        ...
                        if value_map[0]['value_map'] == "" or value_map[0]['value_map'] == 0 or value_map[0][
                            'value_map'] == None:
                            sql = "select avg(time_use_character), avg(time_use_scenario),avg(vote_star)," \
                                  "variance(time_use_character),variance(time_use_scenario),variance(vote_star) from records where code='%s'" % (
                                code)
                            cursor.execute(sql)
                            ...
                            sql="select nlp_error+net_error from task where code='%s'" %(code)
                            cursor.execute(sql)
                            error_count=cursor.fetchall()
                            error_count=error_count[0]['nlp_error+net_error']
                            ...
                            plt.bar(4,error_count,color='r', label='corpus failure')
                            plt.legend()
                        ...
                        for i in range(len(keyword_list)):
                            ...
                            list.append(dict(keyword=keyword_list[i], question=question_list[i], corpus=tmp_list,index=i+1,
                                             count_pro=count_pro))

                        sql = "select avg(time_use_character), avg(time_use_scenario),avg(vote_star)," \
                              "variance(time_use_character),variance(time_use_scenario),variance(vote_star) " \
                              "from records where code='%s'" % (
                            code)
                        cursor.execute(sql)
                        value_list = cursor.fetchall()
                        avg_time_cost_character = value_list[0]["avg(time_use_character)"]
                        ...
                        db.commit()
                        db.close()
                        return render_template("task_detail_mix_layer_off.html",user_list_visual=linear_user_list,
                                               user_list_oral=linear_user_list,code=code,corpus_result=list,avg_score=avg_score\
                                               ,avg_time_cost_character=avg_time_cost_character,
                                               avg_time_cost_scenario=avg_time_cost_scenario,var_score=var_score\
                                               ,var_time_cost_character=var_time_cost_character,
                                               var_time_cost_scenario=var_time_cost_scenario,error_count=error_count)
                ...
                if mode=='agile':
                    ...
                    ###
                        ...
                        if value_map[0]['value_map'] == "" or value_map[0]['value_map'] == 0 or value_map[0][
                            'value_map'] == None:
                            sql = "select avg(time_use_character), avg(time_use_scenario),variance" \
                                  "(time_use_character),variance(time_use_scenario),avg(vote_star),variance(vote_star) " \
                                  "from records where code='%s' and job_type='visual'" % (
                                code)
                            cursor.execute(sql)
                            value_list = cursor.fetchall()
                            avg_time_cost_character = value_list[0]["avg(time_use_character)"]
                        ...
                        sql="select username,vote_star,character_name,time_use_character,time_use_scenario," \
                            "note_step1,note_step2 from records where code='%s' and job_type='visual'" %(code)
                        cursor.execute(sql)
                        ...
                        sql = "select avg(time_use_character), avg(time_use_scenario),variance(time_use_character)," \
                              "variance(time_use_scenario),avg(vote_star),variance(vote_star) " \
                              "from records where code='%s' and job_type='visual'" % (
                            code)
                        cursor.execute(sql)
                        value_list = cursor.fetchall()
                        ...
                        db.commit()
                        db.close()
                        return render_template("task_detail_mix_layer_off.html",user_list_visual=agile_user_list_visual,
                                               user_list_oral=agile_user_list_oral,code=code,corpus_result=list,avg_score=avg_score\
                                               ,avg_time_cost_character=avg_time_cost_character,
                                               avg_time_cost_scenario=avg_time_cost_scenario,var_score=var_score\
                                               ,var_time_cost_character=var_time_cost_character,
                                               var_time_cost_scenario=var_time_cost_scenario,error_count=error_count)
                if mode=='pure_visual':
                    ...
                    sql="select username,vote_star,character_name,time_use_character,time_use_scenario,note_step1," \
                        "note_step2 from records where code='%s' and job_type='pure_visual'" %(code)
                    cursor.execute(sql)
                    ...
                    db.commit()
                    db.close()
                    return render_template("task_detail_mix_layer_off.html",user_list_visual=pure_visual_user_list,
                                           code=code,avg_score=avg_score\
                                           ,avg_time_cost_character=avg_time_cost_character,
                                           avg_time_cost_scenario=avg_time_cost_scenario,var_score=var_score\
                                           ,var_time_cost_character=var_time_cost_character,
                                           var_time_cost_scenario=var_time_cost_scenario)
            else:##mix layer mode
                ...
                if value_map[0]['value_map']=="" or value_map[0]['value_map']==0 or value_map[0]['value_map']==None:
                    ...
                    sql = "select avg(time_use_layer1),variance(time_use_layer1) from records " \
                          "where code='%s' and job_type in('layer_1','layer1_2')" % (
                        code)
                    cursor.execute(sql)
                    ...
                    sql = "select avg(time_use_layer2),variance(time_use_layer2) from records" \
                          "where code='%s' and job_type in('layer_2','layer1_2')" % (
                        code)
                    cursor.execute(sql)
                    value_list1 = cursor.fetchall()
                ...
                sql="select username,time_use_layer1,layer_partner from records where code='%s' and " \
                    "job_type in ('layer_1','layer1_2')" %(code)
                cursor.execute(sql)
                layer1_user_list=cursor.fetchall()

                sql="select username,time_use_layer2,layer_partner from records where code='%s' and " \
                    "job_type in ('layer_2','layer1_2')" %(code)
                cursor.execute(sql)
                layer2_user_list=cursor.fetchall()

                sql = "select avg(time_use_layer1),variance(time_use_layer1) from records where code='%s' and " \
                      "job_type in('layer_1','layer1_2')" % (
                    code)
                cursor.execute(sql)
                value_list1 = cursor.fetchall()
                avg_time_layer1 = value_list1[0]["avg(time_use_layer1)"]
                ...
                sql = "select avg(time_use_layer2),variance(time_use_layer2) from records where code='%s' and " \
                      "job_type in('layer_2','layer1_2')" % (
                    code)
                cursor.execute(sql)
                value_list1 = cursor.fetchall()
                avg_time_layer2 = value_list1[0]["avg(time_use_layer2)"]
                ...
                for i in range(len(layer1_user_list)):
                    f=open("static/user/"+layer1_user_list[i]['username']+"/"+code+"/layer1_description.txt",'r')
                    layer1_description=f.read()
                    f.close()
                    layer1_user_list[i]['layer1_description']=layer1_description

                for i in range(len(layer2_user_list)):
                    f=open("static/user/"+layer2_user_list[i]['username']+"/"+code+"/layer2_description.txt",'r')
                    layer2_description=f.read()
                    f.close()
                    layer2_user_list[i]['layer2_description']=layer2_description


                return render_template("task_detail_mix_layer_on.html",layer1_user_list= layer1_user_list,
                                       layer2_user_list=layer2_user_list,code=code,avg_time_layer1=avg_time_layer1\
                                       ,var_time_layer1=var_time_layer1,avg_time_layer2=avg_time_layer2,
                                       var_time_layer2=var_time_layer2)
...
@app.route("/s_r_progress<code>", methods=['GET'])
def s_r_progress(code):
    if request.method == "GET":
        db = pymysql.connect(host='gz-cynosdbmysql-grp-df9ep7mf.sql.tencentcdb.com',
                             port=25081,
                             user='root',
        ...
        cursor.execute(sql)
        process_count = cursor.fetchall()
        process_count = process_count[0]['process_count']

        sql="select count(username) from records where code='%s' and job_type in ('oral','all')" %(code)
        cursor.execute(sql)
        user_count=cursor.fetchall()
        user_count=user_count[0]['count(username)']

        f = open("static/Task/" + code + "/record/keywords.txt", "r")
        unit_count=len(f.readlines())

        total_count=unit_count*user_count
        print(total_count)
        progress_value=int(float(process_count/total_count*100))



        sql="select speech_recognition from task where code='%s'" %(code)
        cursor.execute(sql)
        sr_value=cursor.fetchall()

        sr_value=sr_value[0]['speech_recognition']
        if sr_value!=2:
            db.commit()
            db.close()
            return render_template('progress_bar.html', code=code, progress_value=progress_value,
                                   total_count=total_count,process_count=process_count)
        else:
            db.commit()
            db.close()
            return redirect(url_for("task_detail",code=code))
